...
/**
  ...
  */
object PingerParent {
  case class Init(self: TAddress, ponger: TAddress, timeout: FiniteDuration) extends KompicsInit[PingerParent]
...
class PingerParent(init: PingerParent.Init) extends ComponentDefinition {
  ...
  val pinger: Component = create(classOf[Pinger], Pinger.Init(init.self, init.ponger, init.timeout))
